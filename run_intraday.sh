#!/bin/bash
# run_intraday.sh — Portfolio Automation Engine wrapper (V4.1)
# Steps per S3-OPS-01 spec, S3-OPS-06 pre-run pull inserted at step 6

set -uo pipefail

BASE_DIR="/opt/dev/universe_SS"
LOG_FILE="/var/log/portfolio/intraday.log"
...
python3 "$BASE_DIR/price_action_intraday.py" >> "$LOG_FILE" 2>&1
SCRIPT_EXIT=$?
...
if [ "$SCRIPT_EXIT" -ne 0 ]; then
    curl -s --form-string "token=${PUSHOVER_TOKEN}" \
         ...
         --form-string "message=Intraday script failed $(date '+%Y-%m-%d %H:%M:%S')" \
